...
Using the data model, if you enter [.in]`SEA`, [.in]`KSEA`, or [.in]`Seattle`, the typeahead directive gives you the option to select [.uicontrol]*Seattle Tacoma Intl.* To see the results of the query, watch the terminal window in which the Node application is running.
