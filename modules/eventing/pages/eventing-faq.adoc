...
* What should be the clock behavior while using timers?
+
During system startup, all clocks in the cluster nodes need to be synchronized. Post-startup, cluster nodes must be periodically synchronized using clock synchronization tools such as Network Time Protocol (NTP).
