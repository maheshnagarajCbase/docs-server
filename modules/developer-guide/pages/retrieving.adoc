...
This API call will return the document along with metadata (such as the xref:cas-concurrency.adoc#concept_iq4_bts_zs[CAS] value).
...
The xref:cli-overview.adoc#concept_lg3_dhm_zs[cbc] tool may be used to fetch documents by their IDs

 $ cbc cat docid
 docid                CAS=0x8234c3c0f213, Flags=0x0. Size=16
 {"json":"value"}

Will output the document’s value and metadata (xref:cas-concurrency.adoc#concept_iq4_bts_zs[CAS] and xref:transcoders.adoc#table_wrc_2nb_bt[flags]).
...
Couchbase Server offers a [.api]`getAndTouch` operation which retrieves an item and updates its xref:expiry.adoc#concept_o53_kps_zs[expiration time] while doing so.
This is most useful to prolong the lifetime of transient data, and delay its expiration a bit further.
See xref:expiry.adoc#concept_o53_kps_zs[TTL] for more information.
