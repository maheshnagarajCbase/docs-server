...
+
Example: "[.input]``light beer``" performs a <<match-phrase-query,Match Phrases Query>> for the phrase "[.code]``light beer``".
...
+
Example: [.input]`description:water` performs a <<match-query,Match Query>> for the term "[.code]``water``", in the `description` field.
