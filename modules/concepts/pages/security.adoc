...
xref:security:concepts-rba.adoc[Role-Based Access Control (RBAC) for Administrators] requiresxref:security:security-ldap-new.adoc[configuration with LDAP]so that the administrator's user ID in LDAP can be mapped by the Full Administrator to one or more fixed administrative roles.
